		...
		success: function (data) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				if (data[i].parent_comment == 0) {
					...
					var row = $('<tr><td><img src="avatar.jpg" width="30px" height="30px">' + data[i].student + ' :<i> ' + data[i].date + ':</i><p style="padding-left:80px">' + data[i].post + '<a data-toggle="modal" data-id="' + commentId + '" title="Add this item" class="open-ReplyModal" href="#ReplyModal">Reply</a>' + '</p></td></tr>');
					$('#record').append(row);
					for (var r = 0; (r < data.length); r++) {
						if (data[r].parent_comment == commentId) {
							var comments = $('<tr><td style="padding-left:80px"><img src="avatar.jpg" width="30px" height="30px" />' + data[r].student + ' :<i> ' + data[r].date + ':</i><p style="padding-left:40px">' + data[r].post + '</p></td></tr>');
